...
|=======================================================================
|  Новая ветка:   getOffersProduct
|  (перенесена из второго скрипта offers.product.php)
|-----------------------------------------------------------------------
*/
elseif ($_GET["act"] == "getOffersProduct") {
    // Параметры: params (json), groupID, page
    if (!empty($_GET["params"]) && !empty($_GET["groupID"]) && isset($_GET["page"])) {

        // Декодируем JSON‑параметры компонента (ожидаем UTF‑8)
        $arParams = \Bitrix\Main\Web\Json::decode($_GET["params"], true);

        if (!empty($arParams)) {
            $arParams["AJAX"]     = "Y";
            $arParams["GROUP_ID"] = $_GET["groupID"];   // all | 0‑9
            $arParams["PAGE"]     = intval($_GET["page"]);

            // Буферизируем вывод компонента
            ob_start();
            $APPLICATION->IncludeComponent(
            ...
            echo \Bitrix\Main\Web\Json::encode([
                "ERROR"   => "Y",
                "MESSAGE" => "params: json_decode false, please check charset (utf8 only)"
            ]);
        }
    } else {
        echo \Bitrix\Main\Web\Json::encode([
				...
				if(!empty($_GET["phone"]) && !empty($_GET["id"])){
					...
					if(CModule::IncludeModule("iblock") && CModule::IncludeModule("sale") && CModule::IncludeModule("catalog")){
						...
						if(!empty($arElement)){
							...
							curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(['oneclick' => $orderData]));
