...
{
	$arViewsToDelete = array();
	$rsViews = $nodeDB->Query("SHOW FULL TABLES IN ".$nodeDB->DBName." WHERE TABLE_TYPE LIKE 'VIEW'", false, '', array("fixed_connection"=>true));
	...
	else
	{
		...
		while ((!empty($arViewsToDelete)) && ($etime > time()))
		{
			...
			$nodeDB->Query("drop view ".$view_name, false, '', array("fixed_connection"=>true));
		}
