...
class Basket{
    ...
    public static function addProductsInfo($arItems){
        ...
        if(!empty($arProductsId)){
            ...
            while($nextElement = $res->GetNextElement()){
                ...
                //next
                $arProduct["PRORERTIES"] = $nextElement->GetProperties();
    ...
    public static function getOrderProperties(){
        ...
        //set location
        self::setOrderLocation($order);

        //get property collection
        $propertyCollection = $order->getPropertyCollection();

        //get group properties
        $arProperties["GROUPS"] = $propertyCollection->getGroups();

        //each properties
        foreach($propertyCollection as $propertyItem){

            //check util
            if($propertyItem->isUtil()){
                continue;
            }
            ...
            $arProperty["CURRENT_VALUE"] = $propertyItem->getValue();

            //location
            if($arProperty["TYPE"] == "LOCATION"){
                $arProperty["LOCATION_ID"] = !empty($arProperty["CURRENT_VALUE"]) ? \CSaleLocation::getLocationIDbyCODE($arProperty["CURRENT_VALUE"]) : self::getUserLocation();
            ...
            if($isAuthorized){
                ...
                elseif($arProperty["IS_PROFILE_NAME"] == "Y" || $arProperty["IS_PAYER"] == "Y"){
                    $arProperty["CURRENT_VALUE"] = $USER->GetFullName();
                }
            ...
            //save result
            $arProperties["PROPERTIES"][$arProperty["ID"]] = $arProperty;

        }

        //save static
        if(!empty($arProperties)){
            self::$basketProperties = $arProperties;
        }
    ...
    public static function setFields($siteId, $deliveryId, $paysystemId, $personTypeId, $locationId){
        ...
        if($order instanceof \Bitrix\Sale\Order){
            ...
            if($propEmail instanceof \Bitrix\Sale\PropertyValue){
                ...
                if(!empty($email)){
                    ...
                    $dbUserLogin = \CUser::GetByLogin($email);
                    ...
                    if(!$dbUserLogin->Fetch()){
                        ...
                        //fail registration
                            ...
                            self::setError($rsRegister->LAST_ERROR);
                        }

                    }

                    else{
                        //C1_NEW_USER_REGISTRATION_LOGIN_ERROR
                        self::setError(\Bitrix\Main\Localization\Loc::GetMessage("C1_NEW_USER_REGISTRATION_LOGIN_ERROR"));
                    }
