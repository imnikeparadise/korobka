<?
	...
	if($arParams["CACHE_TYPE"] != "N" && $obExtraCache->InitCache((int)$arParams["CACHE_TIME"], serialize($cacheID), $cacheDir)){
		//get info by cache
	...
	//check include modules
	if(!\Bitrix\Main\Loader::includeModule("dw.deluxe") || !\Bitrix\Main\Loader::includeModule("sale") && !\Bitrix\Main\Loader::includeModule("iblock")){
		ShowError("modules not installed!");
		return 0;
	}
	...
	$extraContent = DwItemInfo::get_extra_content((int)$arParams["CACHE_TIME"], $arParams["CACHE_TYPE"], $cacheID, $cacheDir, $extraParams, $arParams, $arResult, $opCurrency);
