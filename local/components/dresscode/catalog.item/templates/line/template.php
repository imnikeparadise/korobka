				...
				<table class="prop"><?$i= 0;?>
					<tbody>
						<?foreach($arParams["PRODUCT_DISPLAY_PROPERTIES"] as $nextPropCode):?>
							<?if(!empty($arResult["DISPLAY_PROPERTIES"][$nextPropCode]["DISPLAY_VALUE"]) && $arResult["DISPLAY_PROPERTIES"][$nextPropCode]["SORT"] <= 5000 && $nextPropCode["CODE"] != "CML2_AVAILABLE"):?>
								<?if($i++ == 5){ $i = 0; break;	}?>
								<?
				...
				<?if(!empty($arResult["PRICE"])):?>
					...
					<?else:?>
						<a class="price"><?=CCurrencyLang::CurrencyFormat($arResult["PRICE"]["DISCOUNT_PRICE"], $arResult["EXTRA_SETTINGS"]["CURRENCY"], true)?>
                            ...
                            <?
                            $price = $arResult["PRICE"]["DISCOUNT_PRICE"] / $arResult['PROPERTIES']['KOLPACHEK']['VALUE'];
                            $priceFormat = CCurrencyLang::CurrencyFormat($price, $arResult["EXTRA_SETTINGS"]["CURRENCY"], true);
                            ?>


                                <br/>
                                <span class=" price_p"><?=$priceFormat?> <?if(!empty($arResult['PROPERTIES']['edizm']['VALUE_ENUM'])):?>/<?=$arResult['PROPERTIES']['edizm']['VALUE_ENUM']?><?endif;?></span>
				...
				<?if(!empty($arResult["PRICE"])):?>
					<?if($arResult["CATALOG_AVAILABLE"] != "Y"):?>
						<?if($arResult["CATALOG_SUBSCRIBE"] == "Y"):?>
							<a href="#" class="addCart subscribe" data-id="<?=$arResult["ID"]?>"><img src="<?=SITE_TEMPLATE_PATH?>/images/subscribe.png" alt="" class="icon"><?=GetMessage("PRODUCT_SUBSCRIBE_LABEL")?></a>
						<?else:?>
