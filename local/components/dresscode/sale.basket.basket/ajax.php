...
// --- n8n webhook integration: отправка состава заказа, суммы и покупателя ---
$orderId = false;
if (is_array($orderResult) && isset($orderResult['orderId'])) {
...
if ($orderId) {
    ...
    while ($arItem = $dbBasketItems->Fetch()) {
        ...
        $orderSum += $arItem['PRICE'] * $arItem['QUANTITY'];
    }

    // Получаем свойства покупателя
    $dbOrderProps = CSaleOrderPropsValue::GetList(
        [],
        ["ORDER_ID" => $orderId]
    );
    while ($arProp = $dbOrderProps->Fetch()) {
        $buyerInfo[$arProp['CODE']] = $arProp['VALUE'];
    }
}

$webhookUrlProd = 'https://primary-production-0439.up.railway.app/webhook/96e79d21-fabe-4e3d-b1aa-75fd7c20e70f';
$webhookUrl = $webhookUrlProd;
if (
    (isset($_REQUEST['test']) && $_REQUEST['test'] == '1') ||
    (isset($arTransmitted['test']) && $arTransmitted['test'] == '1')
) {
...
$orderData = [
    ...
    'basket' => $basketItems,
    'order_sum' => $orderSum,
    'buyer' => $buyerInfo, // <-- теперь в n8n будет приходить info о покупателе
];
...
curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 2);
curl_setopt($ch, CURLOPT_TIMEOUT, 3);
// Для теста отключаем проверку SSL (если вдруг проблема в сертификате)
