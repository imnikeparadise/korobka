		...
		if($actionType == "orderMake"){
			...
			if(!empty($arTransmitted)){
	        	...
	        	if($orderResult = $basketAjax->orderMake($arTransmitted)){
	        		...
	        		$webhookUrlTest = 'https://primary-production-0439.up.railway.app/webhook-test/96e79d21-fabe-4e3d-b1aa-75fd7c20e70f';
...
$orderData = [
    ...
    'order_sum' => $arElement["~PRICE"],
	        		...
	        		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 2); // 2 секунды на соединение
	        		curl_setopt($ch, CURLOPT_TIMEOUT, 3);        // 3 секунды на весь запрос
	        		curl_exec($ch);
	        		curl_close($ch);
