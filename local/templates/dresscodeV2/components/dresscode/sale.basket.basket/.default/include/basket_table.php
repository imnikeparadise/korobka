			...
			<tr class="basketItemsRow parent" data-product-iblock-id="<?=$arElement["IBLOCK_ID"]?>" data-id="<?=$arElement["ID"]?>" data-cart-id="<?=$arElement["ID"]?>">
				...
				<td>
					<a class="price">
                        ...
                        $kol = 0;
                        $ed = false;
                        $db_props = \CIBlockElement::GetProperty(15, $arElement['ID'], array("sort" => "asc"), Array("CODE"=>"KOLPACHEK"));
                        if($ar_props = $db_props->Fetch())
                            $kol = $ar_props["VALUE"];

                        $db_props = \CIBlockElement::GetProperty(15, $arElement['ID'], array("sort" => "asc"), Array("CODE"=>"edizm"));
                        if($ar_props = $db_props->Fetch())
                            $ed = $ar_props["VALUE_ENUM"];

                        $price = $arElement["PRICE"] / $kol;
                        $priceFormat = CCurrencyLang::CurrencyFormat($price, 'RUB', true);
                        ?>

                        <span class=" price_p" style="display: block;"><?=$priceFormat?> <?if(!empty($ed)):?>/<?=$ed?><?endif;?></span>
